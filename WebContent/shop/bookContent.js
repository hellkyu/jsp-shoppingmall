$(document).ready(function(){
	$("#insertCart").click(function(){//[장바구니에 담기]버튼 클릭
		...
		$.ajax({
 		     type: "POST",
 		     url: "/jsp-shoppingmall/insertCart.do",
 		     data: query,
	...
	$("#list").click(function(){//[목록으로]버튼 클릭
		window.location.href = "/jsp-shoppingmall/list.do?book_kind=all";
	});
	
	$("#shopMain").click(function(){//[메인으로]버튼 클릭
		window.location.href = "/jsp-shoppingmall/index.do";
	});
	...
	$("#writeQna").click(function(){//[상품QnA쓰기]버튼 클릭
		...
		var book_kind = $("#book_kind").val();
 		...
 		var query="/jsp-shoppingmall/qnaForm.do?book_id="+book_id;
...
function edit(editBtn){//[수정]버튼 클릭
	...
	var arr = rStr.split(",");
	var query = "/jsp-shoppingmall/qnaUpdateForm.do?qna_id="+arr[0];
...
function del(delBtn){//[삭제]버튼 클릭
    ...
    $.ajax({
       type: "POST",
       url: "/jsp-shoppingmall/qnaDeletePro.do",
       data: query,
       success: function(data){
    	   var str1 = '<p id="ck">';
 		   ...
 		   if(check == "1"){//
 		  	  alert("QnA가 삭제 되었습니다.");
 		 	  var query = "/jsp-shoppingmall/bookContent.do?book_id="+arr[1];
